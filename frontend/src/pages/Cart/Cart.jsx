...
import { useDispatch, useSelector } from 'react-redux';
import { useEffect, useState } from 'react';
import { getUserCart, totalUserCartQuantity, resetCartQuantity } from "../../redux/cart/cartAction";
...
const Cart = () => {
  const dispatch = useDispatch();
  const { cart, totalCartQuantity } = useSelector(state => state.cart);
  const { currentUser, isAuthentication } = useSelector(state => state.user);
  const [totalPrice, setTotalPrice] = useState(0);
  const [loading, setLoading] = useState(false);
  const gst = 0;
  ...
  const handleCheckout = async () => {
    try {
      ...
      if (res.data.url) {
        // If the order is successfully placed, reset the cart quantity to 0
      ...
      <div className="small-container cart-page">
        {isAuthentication ? (
          <>
                    ...
                    <tr key={i}>
                      ...
                      <td>
                        <input
                          ...
                          max={item?.book?.stock}
                          onChange={(e) => updateCartQuantity(e, item?.book?._id)}
                        />
                      </td>
                      <td>
                        <button className='btn' onClick={() => removeItemFromCart(item?.book?._id)}>
                          Remove
                        </button>
                      </td>
                      <td>Rs {item?.book?.price}</td>
            ...
            <div className="btn-row">
              <button className='btn mr-2' onClick={clearCart}>
                Clear Cart
              </button>
              <button className='btn mr-2' onClick={handleCheckout}>
                {loading ? (
                  <Hourglass
                    visible={true}
                    height="20"
...
const Cart = () => {
  ...
  const updateCartQuantity = async (e, id) => {
    const newQuantity = parseInt(e.target.value, 10);
    try {
      const { data } = await axios.patch(`/api/v1/cart/quantity`, {
        bookId: id,
  ...
  const clearCart = async () => {
    try {
      const { data } = await axios.delete(`/api/v1/cart/delete`, {
  ...
  const handleCheckout = async () => {
    try {
      setLoading(true);
      const res = await axios.post('/api/v1/order/checkout', {
        withCredentials: true,
        cart,
        currentUser,
      });
      ...
      if (res.data.url) {
        // If the order is successfully placed, reset the cart quantity to 0
        dispatch({ type: 'RESET_CART_QUANTITY' });

        // Clear the cart immediately
        await axios.delete(`/api/v1/cart/delete`, { withCredentials: true });
                    ...
                    <tr key={i}>
                      ...
                      <td>
                        <input
                          ...
                          max={item?.book?.stock}
                          onChange={(e) =>
                            updateCartQuantity(e, item?.book?._id)
                          }
                        />
                      </td>
                      <td>
